...
def preprocessData():
    ...
    columns_to_select = ["Gender", "Age", "Occupation", "City_Category", "Marital_Status"]
    input_df = df[columns_to_select].copy()

    # Function to extract numerical age
    ...
    input_df['Age'] = input_df['Age'].apply(extract_age)

    # Define a dictionary to map City_Category values to numeric values
    city_mapping = {"A": 1, "B": 2, "C": 3, "D": 4}

    # Apply the mapping to the 'City_Category' column
    input_df["City_Category"] = input_df["City_Category"].map(city_mapping)

    # Define a dictionary to map Gender values to numeric values
...
def loadModel():
    ...
    print("---Model loaded---")
    user_input = []
    for column in ["Gender", "Age", "Occupation", "City_Category", "Marital_Status"]:
        while True:
